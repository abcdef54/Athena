...
async def test_conversation_routes(client: AsyncClient, db_session: AsyncSession):
    ...
    detail_response = await client.get(f"/conversation/{conv_id}")
    assert detail_response.status_code == 200
    detail_data = detail_response.json()
    assert detail_data["title"] == "Route Test Thread"

    # 4. Rename conversation
    rename_response = await client.patch(f"/conversation/{conv_id}?new_name=Renamed%20Thread")
    assert rename_response.status_code == 200
    rename_data = rename_response.json()
    assert rename_data["title"] == "Renamed Thread"

    # 5. Fetch messages (should be empty initially)
    msg_response = await client.get(f"/conversation/{conv_id}/messages")
    assert msg_response.status_code == 200
    assert msg_response.json() == []

    # 6. Fetch sources for a non-existent message
    sources_response = await client.get(f"/conversation/{conv_id}/messages/{uuid.uuid4()}/sources")
    ...
    # Verify deleted
    check_response = await client.get(f"/conversation/{conv_id}")
