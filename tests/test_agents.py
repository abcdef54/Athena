...
async def test_get_localmind_system_instruction_personalities():
    from src.backend.agents.core import get_localmind_system_instruction
    
    # 1. Test General baseline / custom file
    general_prompt = get_localmind_system_instruction("general")
    assert "You are LocalMind" in general_prompt

    # 2. Test Coder personality
    coder_prompt = get_localmind_system_instruction("coder")
    assert "LocalMind Code Architect" in coder_prompt

    # 3. Test Gen-Z personality
    genz_prompt = get_localmind_system_instruction("genz")
    assert "LocalMind Gen-Z" in genz_prompt

    # 4. Test Researcher personality
    researcher_prompt = get_localmind_system_instruction("researcher")
    assert "LocalMind Research Analyst" in researcher_prompt

    # 5. Test Unhinged personality
    unhinged_prompt = get_localmind_system_instruction("unhinged")
    assert "LocalMind Unbound" in unhinged_prompt

    # 6. Test Assistant personality
    assistant_prompt = get_localmind_system_instruction("assistant")
    assert "LocalMind Executive Assistant" in assistant_prompt

    # 7. Test Fallback
    fallback_prompt = get_localmind_system_instruction("invalid-personality-name")
    assert "You are LocalMind" in fallback_prompt
